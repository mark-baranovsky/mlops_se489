...
- **Project Lead:** Stack Overflowers (aahme147@depaul.edu)
- **Team Members:** *To be filled in*

## Project Overview

Retail Demand Forecasting is a machine learning project that implements Weekly retail product demand forecasting MLOps pipeline.
...
This template uses the modern **`src/` layout** — the importable package lives in `src/mlops_se489/`, decoupled from the repository root. That forces `pip install -e .` before imports work, which catches packaging bugs early.

```
mlops_se489/                  # Repository root
├── src/
│   └── mlops_se489/          # Importable Python package
...
Data and model artifacts are accessed via the constants in `mlops_se489.config` (`PROJECT_ROOT`, `DATA_DIR`, `MODELS_DIR`, …) rather than relative paths — code is independent of where you invoke it from.
