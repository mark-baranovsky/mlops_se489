...
weekly_returns = (
    returns.groupby(group_cols)["order_demand"]
    .agg(weekly_return_demand="sum")
    .reset_index()
)

weekly_all = (
    df.groupby(group_cols)["order_demand"]
    .agg(weekly_net_demand="sum", order_count="count")
    .reset_index()
)
...
df_gold = df_gold.merge(
    has_returns[group_cols + ["has_returns"]], on=group_cols, how="left"
)
...
df_gold = df_gold.sort_values(
    ["warehouse", "product_code", "week_start_date"]
...
total = len(df_gold)
distinct_keys = (
    df_gold[["warehouse", "product_code", "week_start_date"]].drop_duplicates().shape[0]
)
print(f"\n  Total rows    : {total:,}")
...
print(
    df_gold.groupby("warehouse")["weekly_order_demand"]
    .agg(["count", "sum", "mean"])
    .to_string()
)
